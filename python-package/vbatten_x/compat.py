...
def try_import(name: str):
    ...
    try:
        ...
        raise ImportError(f"{name} is required for this interop function: pip install {name.split('.')[0]}")
...
def field_params_to_numpy(booster, stage: int = -1) -> np.ndarray:
    ...
    if booster._last_model_json is None:
        raise RuntimeError("Call booster.save() before accessing field params")
    data   = json.loads(booster._last_model_json)
    stages = data.get("stages", [])
    idx    = stage if stage >= 0 else len(stages) + stage
    return np.array(stages[idx].get("params", []), dtype=np.float32)
